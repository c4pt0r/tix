...
func pub(streamName string, message string) {
	err := hub.Publish(streamName, &pubsub.Message{
		Data: message,
...
func sub(streamName string, offset pubsub.Offset) {
...
func main() {
	...
	var err error
	hub, err = pubsub.NewPubSub(cfg)
	if err != nil {
		log.Fatal(err)
	}
	// set shell prompts
	shell := ishell.New()
	shell.SetPrompt("pubsub> ")
	...
	shell.AddCmd(&ishell.Cmd{
		...
		Func: func(c *ishell.Context) {
			if len(c.Args) == 1 {
				...
				sub(c.Args[0], pubsub.Offset(offset))
				c.Println("OK")
			} else {
				c.Println("usage: subscribe <streamName> [offset]")
			}
